    ...
    <div class="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        ...
        <nav class="mt-5 px-4 py-4 lg:mt-9 lg:px-6" x-data="{ selected: $persist('Dashboard') }">
            ...
            @foreach ($menuItems as $menuItem)
                <div>
                    @if ($menuItem->title)
                        <h3 class="mb-4 ml-4 text-sm font-medium text-bodydark2">{{ $menuItem->title }}</h3>
                    @endif
                    ...
                    <ul class="mb-6 flex flex-col gap-1.5">
                        <li>
                            <a class="group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4"
                                href="{{ $menuItem->url ?? '#' }}"
                                @click.prevent="selected = (selected === '{{ $menuItem->id }}' ? '':'{{ $menuItem->id }}')"
                                :class="{ 'bg-graydark dark:bg-meta-4': (selected === '{{ $menuItem->id }}') }">
                                ...
                                @if ($menuItem->children->count() > 0)
                                    <svg class="absolute right-4 top-1/2 -translate-y-1/2 fill-current"
                                        :class="{ 'rotate-180': (selected === '{{ $menuItem->id }}') }" width="20"
                            ...
                            @if ($menuItem->children->count() > 0)
                                <div class="translate transform overflow-hidden"
                                    :class="(selected === '{{ $menuItem->id }}') ? 'block' : 'hidden'">
